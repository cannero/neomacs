...
pub use background::{
    ...
    BackgroundWorkerError, BackgroundWorkerStats, SharedBackgroundError, SharedBackgroundService,
    SharedHeap, SharedHeapError,
};
...
mod tests {
    ...
    fn alloc_auto_triggers_major_collection_under_pinned_pressure() {
        let pinned_bytes =
            estimated_allocation_size::<PinnedLeaf>().expect("pinned allocation size");
        let mut heap = Heap::new(HeapConfig {
    ...
    fn major_region_candidates_respect_limit_and_sort_by_hole_bytes() {
        ...
        assert!(
            plan.estimated_reclaim_bytes >= candidates[0].hole_bytes + candidates[1].hole_bytes
        );
    }
