...
- GNU-style match-data restoration is now explicit in the search layer: `MatchData` tracks whether buffer positions are Emacs bytes or character positions, Lisp-string buffer regexp searches now drive the matcher with raw buffer bytes / byte positions, and `match-data`, `set-match-data`, `match-beginning`, `match-end`, `match-string`, and `replace-match` now round-trip multibyte buffer matches like GNU instead of leaking byte offsets through the Lisp boundary.
...
- Focused verification for the final GNU search/match-data cleanup passed on April 19, 2026:
  `cargo nextest run -p neovm-core bootstrap_runtime_set_match_data_restores_multibyte_buffer_positions_like_gnu byte_position_and_clear_bitmap_semantics byte_position_helpers_count_multibyte_raw_bytes`
- Broad verification on April 19, 2026 still reports unrelated existing failures outside this plan's touched surfaces:
  `cargo nextest run -p neovm-core` now passes the legacy byte-position / `set-match-data` regressions and stops only in pre-existing placeholder/runtime-state tests such as `pure_dispatch_record_query_placeholders_match_compat_contracts`, `pure_dispatch_reconsider_redirect_placeholders_match_compat_contracts`, and `pure_dispatch_unicode_value_placeholder_cluster_matches_compat_contracts`. `cargo nextest run -p neomacs-tui-tests` was already carrying existing TUI interaction/layout failures before this final slice.
