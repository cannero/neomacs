...
fn builtin_skip_syntax_forward_limit_uses_char_positions_for_multibyte_text() {
    ...
    assert_val_eq!(moved, Value::fixnum(2));
    assert_eq!(
        eval.buffers
            .current_buffer()
...
fn syntax_table_p_recognizes_syntax_tables() {
    ...
    assert_val_eq!(is_syntax, Value::T);
    ...
    assert_val_eq!(not_syntax, Value::NIL);

    let atom = builtin_syntax_table_p(vec![Value::fixnum(1)]).unwrap();
    assert_val_eq!(atom, Value::NIL);
}
...
fn forward_comment_backward_single_line_comments() {
    ...
    let out = builtin_forward_comment(&mut eval, vec![Value::fixnum(-1)]).unwrap();
    assert_val_eq!(out, Value::T, "forward-comment -1 should return t");
    ...
    assert_val_eq!(out, Value::T, "forward-comment -3 should return t");
    let point_1based = eval
        .buffers
        .current_buffer()
        .expect("current buffer")
...
fn scan_lists_basic_and_backward_nil() {
    ...
    .unwrap();
    assert_val_eq!(forward, Value::fixnum(6));
    ...
    .unwrap();
    assert_val_eq!(backward, Value::NIL);
}
