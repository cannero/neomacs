...
const TRANSIENT_RUNTIME_FEATURES: &[&str] =
    &["cl-macs", "cl-seq", "cl-extra", "gv", "icons", "pcase"];
...
fn normalize_bootstrap_runtime_surface(
    ...
    // calls below. The Values stored in `compile_only_state`,
    // `runtime_loaded_state`, and `runtime_loaddefs_state` come from
    ...
    let roots = eval.save_specpdl_roots();
    for args in compile_only_state
        .autoload_args
        .iter()
        .chain(runtime_loaded_state.autoload_args.iter())
    ...
    for form in compile_only_state
        .keymap_defvar_forms
        .iter()
        .chain(runtime_loaded_state.keymap_defvar_forms.iter())
    ...
    for form in compile_only_state
        .property_forms
        .iter()
        .chain(runtime_loaded_state.property_forms.iter())
        .chain(runtime_loaddefs_state.property_forms.iter())
    {
        eval.push_specpdl_root(*form);
    }
    for replay in compile_only_state
        .keymap_forms
        .iter()
        .chain(runtime_loaded_state.keymap_forms.iter())
...
pub fn apply_runtime_startup_state(eval: &mut super::eval::Context) -> Result<(), EvalError> {
    ...
    // GNU's dumped startup surface retains a loaded `cl-lib` (including
    // `cl--block-wrapper` / `cl--block-throw`) in `load-history`. Source
    // bootstrap cleanup strips the compile-time helper churn above, then we
    // reload the resident runtime `cl-lib` surface explicitly to match GNU.
