...
fn replay_loaded_neobc_forms(
    eval: &mut super::eval::Context,
    source_path: &Path,
    loaded: &super::file_compile_format::LoadedNeobc,
    ...
    eval.with_gc_scope(|ctx| {
        ...
        for form in &loaded.forms {
            let step_result: Result<(), EvalError> = match form {
                ...
                super::file_compile_format::LoadedForm::Constant(_) => Ok(()),
            };
    ...
    replay_result?;
    record_load_history(eval, source_path);
    Ok(Value::T)
}

fn try_replay_neobc_path(
    eval: &mut super::eval::Context,
    source_path: &Path,
    neobc_path: &Path,
    source_hash: &str,
    ...
    match super::file_compile_format::read_neobc_with_surface(
        ...
        Ok(loaded) => {
            ...
            replay_loaded_neobc_forms(eval, source_path, &loaded).map(Some)
        }
        Err(err) => {
            tracing::debug!(
...
fn should_attempt_bootstrap_compile_first(eval: &super::eval::Context) -> bool {
    let dump_mode = eval.visible_variable_value_or_nil("dump-mode");
    dump_mode.as_str() == Some("pbootstrap") || dump_mode.is_symbol_named("pbootstrap")
}

fn reactivate_live_runtime_after_clone(
    eval: &mut super::eval::Context,
    snapshot: &super::pdump::ActiveRuntimeSnapshot,
) {
    super::pdump::restore_active_runtime(eval, snapshot);
}
...
pub(crate) fn eval_decoded_source_file_in_context(
    ...
    for neobc_path in neobc_candidate_paths(path) {
        ...
        if let Some(result) =
            try_replay_neobc_path(eval, path, &neobc_path, &source_hash, expected_surface)?
        {
            return Ok(result);
        }
    }
    ...
    if macroexpand_fn.is_some() && should_attempt_bootstrap_compile_first(eval) {
        let expected_surface = runtime_neobc_cache_path(path)
            .as_ref()
            .filter(|runtime_path| *runtime_path == &auto_neobc_cache_path)
            .map(|_| runtime_surface_fingerprint.as_str());
        if let Some(parent) = auto_neobc_cache_path.parent() {
            let _ = std::fs::create_dir_all(parent);
        }
        let live_runtime = super::pdump::snapshot_active_runtime(eval);
        match super::pdump::clone_active_evaluator(eval) {
            Ok(mut compile_eval) => {
                let compile_result =
                    super::file_compile::compile_el_to_neobc_with_output_and_surface(
                        &mut compile_eval,
                        path,
                        &auto_neobc_cache_path,
                        expected_surface,
                    );
                reactivate_live_runtime_after_clone(eval, &live_runtime);
                drop(compile_eval);
                match compile_result {
                    Ok(()) => {
                        tracing::info!(
                            "bootstrap compile-first saved {} to {}",
                            path.display(),
                            auto_neobc_cache_path.display()
                        );
                        if let Some(result) = try_replay_neobc_path(
                            eval,
                            path,
                            &auto_neobc_cache_path,
                            &source_hash,
                    ...
                    Err(err) => {
                        tracing::debug!(
                            "bootstrap compile-first skipped for {}: {}",
                            path.display(),
                            err
                        );
                    }
            ...
            Err(err) => {
                reactivate_live_runtime_after_clone(eval, &live_runtime);
                tracing::debug!(
                    "bootstrap compile-first clone skipped for {}: {:?}",
                    path.display(),
                    err
                );
            }
            ...
            eager_expand_toplevel_forms(
                ...
                &mut |ctx, original, expanded, requires_eager_replay| {
                    ...
                    ctx.with_gc_scope(|ctx| {
                        ...
                        // Match GNU source loading: cache lowering is a
                        // separate compilation concern. The live load path
                        // still evaluates the source-expanded form, not the
                        // freshly lowered cache artifact. The lowered form is
                        // preserved only for later cache hits until the
                        // compiled same-file runtime path is proven semantically
                        // equivalent for macroexp-heavy files like macroexp.el.
                        ctx.root(expanded);
                        let t4 = std::time::Instant::now();
                        let value = ctx.eval_value(&expanded).map_err(map_flow)?;
                        let d4 = t4.elapsed();
...
fn runtime_neobc_surface_fingerprint(eval: &super::eval::Context) -> String {
    ...
    hasher.update(b"neomacs-runtime-neobc-surface-v2\0");
    for (name, value) in [
        (
            "features",
            eval.obarray()
    ...
    hasher.update(b"load-history-files\0");
    let load_history = eval
        .obarray()
        .symbol_value("load-history")
        .copied()
        .unwrap_or(Value::NIL);
    for entry in list_to_vec(&load_history).unwrap_or_default() {
        let loaded = if entry.is_cons() {
            entry.cons_car()
        } else {
            entry
        };
        hasher.update(super::print::print_value(&loaded).as_bytes());
        hasher.update(b"\0");
    }
